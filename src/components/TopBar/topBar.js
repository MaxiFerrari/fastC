...
const topBar = () => {
  ...
  const toggleRightDrawer = useSelector((state) => state.toggleRightDrawer);
  const [showLogo, setShowLogo] = useState(true);

  const handleToggle = () => {
    dispatch({ type: "TOGGLE_NAVBAR", payload: !toggle });
    setShowLogo(!showLogo);
  };
  const handleToggleRightDrawer = () => {
    dispatch({ type: "TOGGLE_RIGHT_DRAWER", payload: !toggleRightDrawer });
  };
